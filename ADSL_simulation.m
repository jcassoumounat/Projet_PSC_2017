...
[estimated_channel_tot,estimated_channel, estimad_noise] = estimation_test(0.001);
...
while ~isequal(remaining_data, [])
	...
	display("Treatment from frame " frame_nb);
        ...
        %    egal_frame = egalisation(fft(rep_imp), channel_    frame = randi([0,1], sum(log2(allocation_qam)), 1);frame);
        ...
        %% Demodulation %%
        [bitsOut, demodulation_dmt_frame] = demodulation(with_prefixe, alloc); % replace dmt_frame by with_prefixe
