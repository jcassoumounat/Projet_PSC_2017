function [estimated_response, estimated_noise] = estimation_test()
...
%         title('signal with noise');
    ...
    %% Demodulation
        [~, demodulation_dmt_frame] = demodulation(channel_frame, allocation_qam);
        for channel_number = 1 : tot_channel
            ...
            real_qam    = real(cell2mat(qam_frame(channel_number)));
            imag_qam    = imag(cell2mat(qam_frame(channel_number)));
...
%         plot(tableau_temps*Te,noise_frame);
        ...
        for channel_number = 1 : tot_channel
            partial_estimated_noise(channel_number,frame_number) = ...
                partial_estimated_noise(channel_number, frame_number) ...
                + ( ...
                    demodulation_dmt_frame(channel_number) ...
                    - estimated_response(channel_number) .* cell2mat(qam_frame(channel_number)) ...
                   ) ./ tot_frames; 
        end
    end
    ...
    for channel_number = 1 : tot_channel
        for frame_numbre = 1 : tot_frames
            estimated_noise(channel_number) = estimated_noise(channel_number) ...
                                            + partial_estimated_noise(channel_number,frame_number);
        end
    end
...
%     estimated_response
%     figure;
%     plot(tableau_temps2*Te, estimated_response);
    
%     estimated_noise
%     figure;
%     plot(tableau_temps2*Te, estimated_noise);
end
